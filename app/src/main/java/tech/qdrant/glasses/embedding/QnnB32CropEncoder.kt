...
class QnnB32CropEncoder(context: Context) : CropEncoder {
    ...
    private fun runBenchmark() {
        ...
        // Vision — the native all-on-HTP path via EPContext (~24ms/inference on the AR1). Warm up
        // (first run loads the HTP context), then time the full encode() incl. CLIP preprocessing.
        ...
        logStats("vision(NPU W8A16 EPContext)", vms)
