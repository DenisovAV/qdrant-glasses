 ...
 * `src/main` nor both flavors at once, or it would collide). [Backend.OBJECTBOX] and
 * [Backend.SQLITE_VEC] only compile in the benchmark flavor — that's where their engines
 * ([ObjectBoxStore], [SqliteVecStore]) and native libs live — so only THIS copy can reference them.
 * The demo flavor pulls none of it: no ObjectBox/sqlite-vec deps, no `libvec.so`, no extra APK size.
 *
 * [Backend.QDRANT_EDGE] is the product default + baseline. [Backend.OBJECTBOX] and
 * [Backend.SQLITE_VEC] are also fully wired here — the benchmark's HNSW and brute-force-in-SQLite
 * comparison points ([ComparisonBenchmarkTest] builds all three side by side). Only [Backend.USEARCH]
 * and [Backend.FAISS] remain `TODO`, until their phase lands. Flipping [backend] is the ONLY change
 * needed to build+measure a different engine via [VectorStoreBenchmark] — never a runtime switch.
 */
